...
### FORBIDDEN: Bypassing /build for Team-Orchestrated Plans

**This is the #1 enforcement rule for plan execution.**

If a plan contains `## Team Orchestration` OR an `EXECUTION DIRECTIVE` block, it was created by `/plan_w_team` and MUST be executed via `/build`. This is NOT a recommendation — it is a hard requirement.

| If You See... | FORBIDDEN Response | REQUIRED Response |
|---------------|-------------------|-------------------|
| Plan with `## Team Orchestration` | Read files and start editing code directly | Invoke `/build specs/<plan-file>.md` |
| Plan with `EXECUTION DIRECTIVE` | Use `executing-plans` skill | Invoke `/build specs/<plan-file>.md` |
| "Implement the following plan:" + team plan content | Use Edit/Write tools yourself | Invoke `/build specs/<plan-file>.md` |
| Plan mentions TeamCreate, Task agents, team members | Act as a solo builder | Invoke `/build specs/<plan-file>.md` |

**Why this matters**: `/build` has `disallowed-tools: Write, Edit, NotebookEdit` — it physically CANNOT write code. It MUST delegate to team agents. When you bypass `/build` and implement directly, you destroy the entire team orchestration workflow.
...
| **executing-plans** | Executing a solo plan (NO `## Team Orchestration` section) | Step 4 |
| **/build** (command) | Executing a team-orchestrated plan (HAS `## Team Orchestration` section). FORBIDDEN to use `executing-plans` for these. | Step 4 |
